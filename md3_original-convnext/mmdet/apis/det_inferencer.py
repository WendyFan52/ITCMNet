...
class DetInferencer(BaseInferencer):
    ...
    def __call__(
        ...
        for ori_imgs, data in (track(inputs, description='Inference')
            ...
            img_path = osp.basename(data['data_samples'][0].img_path)
        ...
        import cv2
        new_masks = []
        masks = results_dict['predictions'][0]['masks']
        if len(masks) >= 0:
            for i in masks:
                mask = maskUtils.decode(i)
                Image.fromarray(mask * 255)
                contours, hierarchy = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
                boundary_points = contours[0][:, 0, :].tolist()
                new_masks.append(boundary_points)
            results_dict['predictions'][0]['masks'] = new_masks
            out_json_path = osp.join(out_dir, 'preds', img_path + '.json')
            if not no_save_pred:
                mmengine.dump(results_dict, out_json_path)
        else:
            pass
