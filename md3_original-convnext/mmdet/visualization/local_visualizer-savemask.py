    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'masks' in instances:
            ...
            for i, mask in enumerate(masks):
                ...
                mask_rgb = np.stack([mask] * 3, axis=2)  # Expand to a 3-channel mask
                masked_image = image_np * mask_rgb  # Convert the unmasked areas to black
                merged_masked_image += masked_image  # Image after blending masks

            merged_masked_pil = Image.fromarray(merged_masked_image.astype(np.uint8))
        ...
        return self.get_image(),merged_masked_pil
    ...
    def add_datasample(
        ...
        if draw_pred and data_sample is not None:
            ...
            if 'pred_instances' in data_sample:
                ...
                pred_img_data,mask_pil = self._draw_instances(image, pred_instances,
                                                     classes, palette)
        ...
        if out_file is not None:
            ...
            name = os.path.basename(out_file).split('.')[0] + '_result' + '.png'
            save_fold = 'mask'
