...
def main():
    ...
    parser.add_argument('--json_input_dir', default='data/raw_data/json',help='input annotated directory')
    parser.add_argument('--image_input_dir', default='data/raw_data/img',help='image directory')
    parser.add_argument(
        '--output_dir', help='output dataset directory', default='./data/coco')
    parser.add_argument(
        '--train_proportion',
    ...
    if args.dataset_type == 'voc':
        ...
        if args.train_proportion != 0:
            ...
            out_dir = args.output_dir + '/train2017'
        ...
        if args.val_proportion == 0.0:
            ...
            val_out_dir = args.output_dir + '/val2017'
        ...
        for img_name in os.listdir(args.image_input_dir):
            if count <= train_num:
                if osp.exists(args.output_dir + '/train2017/'):
                    shutil.copyfile(
                        osp.join(args.image_input_dir, img_name),
                        osp.join(args.output_dir + '/train2017/', img_name))
            else:
                if count <= train_num + val_num:
                    if osp.exists(args.output_dir + '/val2017/'):
                        shutil.copyfile(
                            osp.join(args.image_input_dir, img_name),
                            osp.join(args.output_dir + '/val2017/', img_name))
                else:
